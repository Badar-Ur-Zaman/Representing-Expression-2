...
public class Multiplication implements Expression {
    ...
    public Expression simplify(Map<String, Double> environment) {
        ...
        if (simplifiedLeft instanceof Number && simplifiedRight instanceof Number) {
            double leftValue = ((Number) simplifiedLeft).getNumber();
            double rightValue = ((Number) simplifiedRight).getNumber();
            return new Number(leftValue * rightValue);
        }

        // Case 2: If either operand is 0, the result is 0 (anything * 0 = 0)
        if (simplifiedLeft instanceof Number && ((Number) simplifiedLeft).getNumber() == 0) {
            return new Number(0); // 0 * x = 0
        }
        if (simplifiedRight instanceof Number && ((Number) simplifiedRight).getNumber() == 0) {
            return new Number(0); // x * 0 = 0
        }

        // Case 3: If either operand is 1, return the other operand (anything * 1 = the other operand)
        if (simplifiedLeft instanceof Number && ((Number) simplifiedLeft).getNumber() == 1) {
            return simplifiedRight; // 1 * x = x
        }
        if (simplifiedRight instanceof Number && ((Number) simplifiedRight).getNumber() == 1) {
